...
class Command(BaseCommand):
    def handle(self, *args, **options):
        time_threshold = datetime.now() - timedelta(hours=1) # change the hours if you like
        accounts = Account.objects.filter(is_active=False, date_joined__lt=time_threshold)
